...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| In Progress | Work | 18A: Single validation entry point | Implemented, reviewer-approved, awaiting commit and CI: `ValidateIndex` and `LocateIndex` in `src/index_state.cpp` return an `IndexVerdict` (availability, reason, meta); the seven read paths call `ValidateIndex` and keep their own policy on a non-empty reason (scan, raise, refuse); `IndexLocationAvailable` and `ReadMeta` are deleted; `RowIdGuardReason` keeps two direct callers for the shared-guard create, which has no registry row yet. Missing: commit hash, run URL. |
| In Progress | Work | 18B: Handle-based execution-time scalars | Implemented, reviewer-approved: the pragma callback stores a `PreparedMaintenance` in the per-connection `MaintenanceState` under a random UUID and the script calls `__ngram_maintenance_guard('<uuid>')`; the scalar consumes the handle and runs the same drop, create, and maintenance checks as the former 14-argument and 5-argument scalars (reviewer compared them check by check). Handles are grouped per script builder so two pragmas in one query string cannot erase each other's handles (regression block `test/sql/ngram_maintenance.test:928-1018`, fail-then-pass); a handle is erased only by its own consumption, by the end of the transaction that consumed a same-group handle, or by a later expansion in another transaction. Remaining assumption: a client does not expand a second pragma string before running the first's statements; no public path does. Missing: commit hash, run URL. |
| In Progress | Work | 18C: Split long functions and collapse overflow declines | Implemented, reviewer-approved: no function over 150 lines (largest `SearchInitGlobal` 111, `CompactScript` 105); `PlanIndexProbe`, create, refresh, and the maintenance scalar are split into named helpers verified as line-for-line extractions; every `CheckedAdd`/`CheckedMultiply` failure throws one `InvalidInputException("ngram: probe arithmetic overflow")`; five decline strings remain, the plan's four plus `query grams exceed query memory budget`, which is reachable under a small memory limit and whose removal would turn a scan fallback into an out-of-memory error. Missing: commit hash, run URL. |
| In Progress | Work | 18D: Module layout | Implemented, reviewer-approved: fifteen sources and eleven headers with one-line responsibility banners; largest file `rewrite.cpp` at 623 lines; refresh and compact scripts live in `maintenance_sql.cpp` so `build_sql.cpp` stays under 700, and `MetaInfo` lives in `catalog.hpp` beside `RegistryRow`, which embeds it; 116 unneeded DuckDB includes removed. Missing: commit hash, run URL. |
...
| In Progress | Work | 18F: Comment, naming, and header hygiene | Implemented, reviewer-approved: DuckDB's `MAX_ROW_ID` replaces the literal; `ScratchName` and `FETCH_BATCHES_PER_SEGMENT` defined once; `SilentGuard`, `RethrowFatalObservation`, `PrepareCreationTransaction`, duplicate base resolvers, and the `VerifyBuffers` override removed; history-narrating comments rewritten; `EncodePostings` is file-local. `GuardTableName`, `ObservedIndex.format_version`, and `ProbePlan::stats_chunks_scanned` stay because they are used. Missing: commit hash. |
| In Progress | Gate | Behavior-neutral restructuring | `make test_release`: 3046 assertions in 25 cases before and after the restructuring, 3158 with the new two-pragma block; seeds identical (1764/0, 886/0, 320/0, 320/0); generated-SQL expansion parity over 13 pragma shapes differs only by the validate statement folded into the handle scalar. Gate deviation, accepted by the coordinator with the reviewer's breakdown: production lines grew from 6533 to 6733 (+200); `.cpp` files shrank by 39 while headers grew by 239 (declarations and shared structs +129, comments +49, blank +54, includes +7), the cost of module boundaries rather than padding. Missing: run URL. |
| In Progress | Test | Suite parity and lazy-bind case | Suite parity above; no lazy-bind case because 18E was rejected with analysis. `make format-check` passed; `git diff --check` clean; `make tidy-check` not run locally (no clang-tidy), covered by the matrix's Tidy Check. Missing: run URL. |
