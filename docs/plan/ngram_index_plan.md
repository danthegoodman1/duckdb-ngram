...
| Complete | Scope | CI has a dedicated correctness/sanitizer/race lane | `.github/workflows/Correctness.yml` adds isolated Release and focused DEBUG/ASAN/UBSan jobs on `ubuntu-24.04`; the existing distribution workflow remains unchanged. Checkout, DuckDB, and extension-ci-tools are immutable-SHA pinned, token permissions are read-only, submodule identities are asserted, and both jobs require a fresh build directory and prove the binaries/configuration they execute. The full 37-minute DEBUG suite remains a manual/scheduled scale gate rather than a pull-request requirement. |
...
| Complete | Work | 14C: Add bounded correctness CI and release-evidence checks | The Release job builds once at parallelism 2, runs the full suite plus artifact consistency/current-binary smoke checks, then the four explicit fixed-seed differential/churn gates. The serial DEBUG job builds only `unittest` and `ngram_checkpoint_gap_test`, asserts the exact sanitizer/reduced-symbol/static-extension cache and link contract plus in-tree DSO resolution, and runs four focused SQL selectors and the deterministic race under fail-fast ASAN/UBSan settings. Every build/test command and both jobs have explicit timeouts; no cache, package install, helper framework, or duplicate build was added. |
| Complete | Gate | Lifecycle is leak-free/collision-free and release claims are traceable | Phase 14A lifecycle/collision, Phase 14B provenance/parity/generated-doc, and Phase 14C bounded correctness gates are complete. A fresh local replay of the exact workflow commands passed: Release build 9:09.80 wall / 1,529,964 KiB peak RSS / 717 MiB tree; serial focused DEBUG build 15:49.12 / 2,762,780 KiB / 6.6 GiB tree; heaviest sanitizer test 4,012,860 KiB. These measurements support the 30-minute Release and 45-minute DEBUG job budgets on the intended 8 GiB RAM / 14 GiB disk runner, but do not claim a GitHub-hosted run before the workflow is committed and dispatched. |
...
| Complete | Test | Artifact-generation and correctness-CI gates | Phase 14B's golden, parser/provenance, normalization, live-binary parity, and canonical-rendering checks remain covered. The Phase 14C local exact-command replay passed the deterministic harness plus the full Release suite (2,899 assertions / 24 SQL cases), release-evidence check and current-binary smoke, focused sanitizer lifecycle/search/rewrite/maintenance selectors (47/134/149/237 assertions), and the deterministic race with no ASAN/UBSan finding. Fixed gates reproduced explicit seed 20261314 (**1,764/0**), transparent seed 20261315 (**886/0**), normal churn seed 20261316 (**320/0/0 states**), and strict churn seed 20261318 with `--no-stale-expected` (**320/0/0 detector verdicts**). |
