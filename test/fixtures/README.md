...
| `format5.duckdb` | 5 (`__ngram.segments_<hex>` with `generation`, guard `__ngram_guard_<hex>`) | written by the format-5 build of d28ccab, 2026-09-08 (Phase 19 repair), which committed it | `docs(s)` with 7 rows (rowids 0 to 6), one index on `s` (gram 3, case-insensitive, mark 4, so rows 5 and 6 are a persisted tail) | listed `READY`; explicit and transparent searches probe and match the oracle across the persisted tail; a refresh advances the mark to 6 exactly; the drop by reference removes every ngram object |
...
Every fixture was written by a build of this extension against the DuckDB
pinned in the `duckdb` submodule, v1.5.5 at storage version 64 (the files'
storage tag reads `v1.0.0+`), so a newer host's storage upgrade is not part of
what these files test. Git records the committing commit; the producing
