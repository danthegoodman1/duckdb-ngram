...
| --- | --- | --- | --- |
| Incomplete | Work | 16A: Remove the empty-insert-then-delete shape from refresh and compact | Missing: fold the delta into the stats rewrite as one `CREATE TEMP TABLE ... AS SELECT ... FROM (stats UNION ALL packed)` so no statement inserts into `stats` before the `DELETE`; audit `CompactNgramIndexQuery` for the same shape when `keys` is empty; regression test green; upstream DuckDB issue link with the plain-table script below. |
| Incomplete | Work | 16B: Latch only on possible reuse | Missing: record the checkpoint iteration at each `max_seen` advance and latch in `Append` only when `start <= max_seen` and the iteration changed since that advance, persisting the iteration with the other guard options; a guard bound from disk stays conservative; two-connection harness case passes; `docs/stale-updates.md` "Guard mutation" paragraph rewritten to the new rule. |
...
| Incomplete | Work | 16C: Read-path fallbacks and small fixes | Missing: `IndexLocationAvailable` callers on `ngram_search` and `NGRAM_INDEX_SCAN` treat "changed" as "absent" and scan; the maintenance pragmas keep the throw; `lock_guard` on `entry.lock` around `HasBufferedReplays`; extension-owned `InternalException` sites (`ngram_rewrite.cpp:338,441`, `ngram_search.cpp:289,1477`, `ngram_maintenance.cpp:316,328`) become `InvalidInputException`; harness case for the prepared-statement path. |
| Incomplete | Work | 16D: CI completes on HEAD | Missing: Correctness job builds within budget (pinned `ccache` action keyed on the `duckdb` gitlink, or a `build/release` cache; parallelism 3); `MainDistributionPipeline.yml` triggers limited to `push: main, tags v*`, `pull_request`, `workflow_dispatch`; one `scripts/verify_pins.sh` used by both jobs and `release_evidence.py`; fixture-drift step (`gen_trigram_fixtures.py` then `git diff --exit-code`); `git rm --cached .claude/scheduled_tasks.lock`; `.gitignore` gains `.claude/`, `.agents/`, `.codex/`, `*.db`, `*.db.wal`; `packaging/SUBMISSION.md` matrix checkbox unticked until a run on HEAD is green. |
| Incomplete | Gate | Queries survive maintenance in-process and CI is green | Missing: `ngram_refresh_noop.test` in the suite; F2 harness case; Correctness run URL on the phase head; matrix run URL or recorded blocker. |
| Incomplete | Test | Regression and workflow evidence | Missing: test file paths, harness case names, `make test_release` assertion count, four seed results, run URLs. |
