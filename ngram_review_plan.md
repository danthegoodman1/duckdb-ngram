...
| Incomplete | Work | 22F: Nightly and focused CI | `Correctness.yml`: the pull-request sanitizer lane runs twelve files (lifecycle, search, rewrite, maintenance, api, gram_key, query_bounds, parallel, batch_order, refresh_concurrent, search_concurrent, long_inputs) plus the harness; a format check (`make format-check`, clang-format 11.0.1, black, cmake-format) runs before the release build; the sanitizer build sets `NGRAM_STRICT_WARNINGS=ON`, a CMake option that compiles the extension's own sources with `-Wall -Wextra -Werror` (nine redundant moves in return statements were the only findings). `cmake_minimum_required(VERSION 3.5...3.29)` matches the host. `Nightly.yml` (06:17 UTC and on dispatch): the drivers at full size on a release build, the whole suite and the harness under ASAN/UBSAN, and the concurrency files plus the query harness under ThreadSanitizer in the host's own configuration (clang, RelWithDebInfo, `duckdb/.sanitizer-thread-suppressions.txt`, as `duckdb/.github/workflows/Main.yml` runs it). Missing: the first recorded nightly run; the row becomes Complete with its URL and outcome. |
| Incomplete | Gate | Tests cover the changed mechanisms | Fail-before evidence: the publication barrier holds segment 0 until segment 1 is pending, which the pre-19D queue emitted out of order; the multiset oracle counts a duplicated accelerated row where `EXCEPT` did not (`ngram_search_differential.test` header); the decode-time interrupt surfaces where a stranded worker would hang the harness. No correctness assertion depends on machine speed (registry-scale timings are reports). Crash coverage runs on format 5 with 0 failures. Missing: the recorded nightly run and the Correctness run on the pushed source; the gate closes with their URLs. |
