...
| --- | --- | --- | --- |
| In Progress | Work | 16A: Remove the empty-insert-then-delete shape from refresh and compact | Implemented, awaiting commit: the packed delta is folded into the single stats rewrite statement (`src/ngram_maintenance.cpp`, validated stats UNION ALL packed rows), so nothing inserts into `stats` before its `DELETE`; compact audited (empty segments insert is never followed by a segments delete). `test/sql/ngram_refresh_noop.test` fails on the old script (`segments and stats descriptor counts disagree`) and passes on the new one. `docs/upstream/duckdb-empty-batch-insert.md` holds the verified plain-table reproduction. Missing: commit hash; filing the upstream DuckDB issue is an owner action (outward-facing), recorded here as open until the owner files it or declines. |
| In Progress | Work | 16B: Latch only on possible reuse | Implemented, reviewer-verified against host source, awaiting commit: each `max_seen` advance records the header checkpoint iteration; an append at or below `max_seen` latches only when that iteration is unknown or changed (`src/rowid_guard.cpp` `ObservableCheckpointIteration`, `Observe`). Nothing new is persisted: a guard bound from disk or WAL stays conservative because the checkpoint that serialized `max_seen` may itself have vacuumed; in-memory databases have no header counter and also stay conservative; a checkpoint between a rejected commit and its retry still latches. Harness `TestRejectedCommitKeepsGuard` (file database, UNIQUE ART after the guard, rejected duplicate commit, fresh insert) leaves `stale_reason` NULL and keeps `NGRAM_INDEX_SCAN`; it fails on the old code. `docs/stale-updates.md` and README describe the rule. Missing: commit hash. |
...
| In Progress | Work | 16C: Read-path fallbacks and small fixes | Implemented, awaiting commit: `IndexLocationAvailable(..., changed_is_absent)` is passed true by `SearchInitGlobal` and `TryProbeIndex` so a changed identity falls back to the exact scan; bind, candidates, stats, and maintenance keep the throw. `entry.lock` is taken around `HasBufferedReplays` (same lock and order as the host's `AppendToIndexes`). The six extension-owned `InternalException` sites are `InvalidInputException`. F4 coverage is the exact-fallback assertion in harness cases `swapped_part` and `registry_swap`; the prepared-LIKE case covers re-create by another connection only. The `NGRAM_INDEX_SCAN` changed branch has no deterministic test because prepared base-table scans re-bind after catalog changes. Missing: commit hash. |
| In Progress | Work | 16D: CI completes on HEAD | Implemented, awaiting commit and a run: pinned `hendrikmuhs/ccache-action` v1.2.24 in both Correctness jobs keyed on the duckdb gitlink and `src/**`; parallelism 3 release, 2 debug; fixture-drift step; `scripts/verify_pins.sh` (commit match plus clean submodule trees) shared by both jobs and `release_evidence.py`; distribution triggers limited to `main`, `v*` tags, pull requests, dispatch; lock file untracked; `.gitignore` extended; SUBMISSION matrix checkbox unticked. Evidence check split: pull-request CI runs `release_evidence.py check` (artifact self-consistency, gitlink, ancestry, rendered-block match); the tag or dispatch job runs `check --current-source` (adds the `src/**` digest and tool hash match), because a source digest tied to a multi-hour benchmark cannot gate every pull request while the README states which commit its numbers describe. Missing: commit hash; Correctness run URL on the phase head; matrix run URL. |
| In Progress | Gate | Queries survive maintenance in-process and CI is green | `test/sql/ngram_refresh_noop.test` (94 assertions) and harness case `TestRejectedCommitKeepsGuard` are in the suite and pass; the Phase 16 reviewer approved after three rounds. Missing: Correctness run URL on the phase head; matrix run URL or recorded blocker (the matrix format check needs `make format` first, applied as a follow-up commit). |
| In Progress | Test | Regression and workflow evidence | `make test_release`: 2993 assertions in 25 cases (harness first, then every `test/sql/*.test` including `ngram_refresh_noop.test` at 94 assertions). Fixed seeds on the same `src/` tree: 20261314 1764 checks 0 failures; 20261315 886/0; 20261316 320/0; 20261318 (`--no-stale-expected`) 320/0. `release_evidence.py check` PASS; `check --current-source` fails with the expected digest mismatch until the next collection; `release_evidence.py test` PASS; `verify_pins.sh` exit 0 here and exit 1 with a probe file in `duckdb/`; `git diff --check` clean. Missing: Correctness run URL; matrix run URL. |
