...
def candidates_sql(needle, case_insensitive, index_ref):
    ...
    return (
        ...
        "SELECT rowid FROM corpus WHERE %s AND rowid <= "
        "(SELECT hwm_rowid FROM __ngram.registry WHERE index_id = %s::UUID)"
        " EXCEPT SELECT rowid FROM ngram_candidates('corpus', 's', %s));"
        % (q, pred, sql_quote(index_ref), q)
    )
...
def one_trial(trial, rng, rows, transparent):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if transparent:
            ...
            phase_checks += [candidates_sql(n, case_insensitive, ref) for n in needles if len(n) >= gram_size]
        ...
        if transparent:
            ...
            update_checks = [differential_sql(UPDATE_MARKER, case_insensitive),
                             candidates_sql(UPDATE_MARKER, case_insensitive, ref)]
