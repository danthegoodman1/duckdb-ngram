...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| In Progress | Work | 17A: Delete legacy v2 | Implemented, reviewer-approved, awaiting commit and CI: every pre-registry naming helper, both legacy branches, `LegacyRowIdGuardReason`, the format-2 validate branch, `ReadMetaFormatVersion`, the `LEGACY_REBUILD` status, and the `kind` and `storage_schema` columns are gone; harness legacy cases deleted. `Hex()` stays for the owner-key literal. Missing: commit hash, run URL. |
| In Progress | Work | 17B: Guard as sole identity | Implemented, reviewer-approved: `TableFingerprint`, `CertainStaleReason`, `InstanceId`, `ThrowIfCandidatesCertainlyStale`, the "index stale" fallback, the fingerprint re-check, `REPLACED`, and five meta columns deleted; `MetaInfo` is options, hwm, column, guard name, guard token. Reviewer confirmed `RowIdGuardReason` runs before the first postings read on `ngram_search`, `ngram_candidates`, `NGRAM_INDEX_SCAN` (execution and plan time), `ngram_index_stats`, refresh, compact, and shared-guard create. Missing: commit hash, run URL. |
| In Progress | Work | 17C: Registry row is the metadata row | Implemented, reviewer-approved: `__ngram.registry` carries the twelve metadata columns; postings and stats live in `__ngram.segments_<hex>` and `__ngram.stats_<hex>`; the meta table, per-index schema, `ReadMetaHeader`, the registry audit, and the schema cleanup are gone; refresh updates `hwm_rowid` in place (non-indexed column, verified against the host); drop is one `DELETE` and two `DROP TABLE`. Statuses are exactly `READY`, `SCAN_ONLY`, `ORPHAN`, `MALFORMED`. Missing: commit hash, run URL. |
| In Progress | Work | 17D: One guard per table | Implemented, reviewer-approved: the first index runs the barrier and creates `__ngram_guard_<hex>` over every VARCHAR present; later indexes validate and record the same guard with no barrier; a column outside the guard's coverage is refused with one message; the guard drops with the last referencing row. Because the reference count reads the dropping transaction's snapshot, two concurrent drops can strand the guard, so a first-index create drops every unreferenced `__ngram_guard_*` on the table before its barrier (harness `TestConcurrentDropsStrandGuard`, fail-then-pass). Protector paths deleted; the maintenance scalar takes 14 arguments instead of 27. Missing: commit hash, run URL. |
...
| In Progress | Work | 17E: Format 4 and drop-only path | Implemented, reviewer-approved: format 4; a six-column registry-version-1 registry is recognized, listed `MALFORMED` with "format 3 ... predates format 4", and dropped by id through one branch; `test/fixtures/format3.duckdb` (258,048 bytes, built with the pre-change binary) is tested read-only in `ngram_lifecycle.test` and writable in harness `TestFormat3Fixture`. Deferred to 21G: list an unreferenced `__ngram_guard_*` as `MALFORMED`; document the drop-versus-create window that yields a born-`SCAN_ONLY` index. Missing: commit hash, run URL. |
| In Progress | Doc | 17F: Correct the vacuum sentence | README and `docs/stale-updates.md` rewritten: any index blocks moving vacuum by default and the non-ART type matters under `vacuum_rebuild_indexes`; guard as identity; one guard per table; four states; the two concurrency windows. Missing: commit hash. |
| In Progress | Gate | Four states, one anchor, 600 fewer lines | `git diff --stat -- src/`: 752 insertions, 1888 deletions, net -1136 production lines. Status literals: 8 in `index_pragmas.cpp`, 0 elsewhere. Guard-verdict call sites listed under 17B. Missing: run URL. |
| In Progress | Test | Lifecycle, shared-guard, and fixture suites | `make test_release`: 3046 assertions in 25 cases (harness includes `TestSharedGuardAndDrop`, `TestConcurrentDropsStrandGuard`, `TestFormat3Fixture`); `ngram_lifecycle.test` 93, `ngram_maintenance_identity.test` 71, `create_index.test` 211 assertions; seeds 20261314 1764/0, 20261315 886/0, 20261316 320/0, 20261318 320/0; `release_evidence.py check` and `test --binary` PASS (the tool accepts the format-3 registry contract of the stored artifact until Phase 23 re-collects); format-check passed; `git diff --check` clean. Missing: run URL. |
