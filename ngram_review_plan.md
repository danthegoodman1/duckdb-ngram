...
| Complete | Work | 22G: Host locks held across threads by the guard build and the creation barrier | The guard installs through its own source operator (`PhysicalCreateRowIdGuard`, `src/rowid_guard.cpp`): one call, on one thread, takes the table's append lock, reads the rowid baseline, creates the catalog entry and calls `DataTable::AddIndex`, so the baseline and the installation stay one window against committed appends and the thread that took the lock releases it. It looks up its transactions before taking the lock, because a commit holds the transaction locks while it takes the append lock. The creation barrier still holds the checkpoint lock across statements; DuckDB 2.0's `StorageLock` keeps its state under a condition variable and holds no mutex between acquire and release, so another thread may release it. The ThreadSanitizer lane runs clean with neither suppressed (`test/tsan-suppressions.txt`). |
