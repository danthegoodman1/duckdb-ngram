...
from common import bench_path, log, mb, require_duckdb, run_sql, write_json  # noqa: E402
...
def build_blocks(db, settings, shadow, table_suffix):
    """One heavy pass: per 32-delta block, its width, count and varint cost."""
    segments = "%s.segments_%s" % (shadow, table_suffix)
    sql = """
...
def main():
    ...
    shadow = "ngram_main_%s" % args.table
    segments = "%s.segments_%s" % (shadow, args.column)
    report = {"db": db}
...
""".format(seg=segments))
    ...
    if not args.skip_build:
        report["analysis_seconds"] = build_blocks(db, settings, shadow, args.column)
