...
| Complete | Work | 22F: Nightly and focused CI | `Correctness.yml` on every push: the release suite with the harness, the fixed-seed drivers, a format check (`make format-check`: clang-format 11.0.1, black, cmake-format), and a focused ASAN/UBSAN lane over ten files (lifecycle, search, rewrite, maintenance, api, gram_key, query_bounds, refresh_concurrent, search_concurrent, long_inputs) plus the harness, built with `NGRAM_STRICT_WARNINGS=ON`, a CMake option that compiles the extension's sources with `-Wall -Wextra -Werror` (nine redundant moves in return statements were the only findings); `cmake_minimum_required(VERSION 3.5...3.29)` matches the host. `Nightly.yml` (06:17 UTC, on dispatch, and on a push that changes it or `test/tsan-suppressions.txt`, since a workflow is dispatchable only once it exists on the default branch; a running nightly is left to finish): the drivers at full size on a release build, the whole suite and the harness under ASAN/UBSAN with a four-hour budget and the log's tail on failure, and the concurrency files plus the query harness under ThreadSanitizer in the host's own configuration (clang, RelWithDebInfo, the host's suppressions plus `test/tsan-suppressions.txt`), advisory as the host's own ThreadSanitizer job is. `ngram_parallel` and `ngram_batch_order` run only in the nightly lanes: each takes more than fifteen minutes under ASAN on a runner. Recorded on 131c21a: Correctness [34436332949](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34436332949) green; nightly [34436333030](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34436333030) green, the ASAN/UBSAN lane passing the whole suite (31 files) and the harness (24 tests) for the fourth consecutive nightly and the advisory lane's remaining reports carrying no extension frame. The first nightlies (34376946011, 34391482959, 34408156219, 34422370712) found the two sanitizer timeouts and three ThreadSanitizer shapes with an extension frame: the guard build's lock order and its append lock released by the client thread, the creation barrier's checkpoint lock released across statements, and the fence's transaction lookup from a scalar; each is suppressed with its reasoning in `test/tsan-suppressions.txt`, and the two cross-thread locks are deferred row 22G. |
| Deferred | Work | 22G: Host locks held across threads by the guard build and the creation barrier | `GuardBuildGlobalState` holds `DataTable`'s append lock from the sink's initialization (a worker thread) until the host destroys the create-index plan (the client thread), so that the rowid baseline and `AddIndex` are one window against committed appends; the creation barrier holds the host's exclusive checkpoint lock (`StorageLockKey`) from the barrier scalar to the finish scalar across the statements that invalidate old storage and install the guard. Both locks are `std::mutex` underneath, released by a thread other than the one that took them, which the standard leaves undefined and glibc's default mutex completes; ThreadSanitizer reports the releases and, for the checkpoint lock, a lock-order inversion with the client context lock that the next statement takes. All three reports are suppressed with attribution in `test/tsan-suppressions.txt`. A fix needs the host to expose append and checkpoint exclusion whose ownership can move between threads, or a barrier that completes within one statement; both are host changes. |
| Complete | Gate | Tests cover the changed mechanisms | Fail-before evidence: the publication barrier holds segment 0 until segment 1 is pending, which the pre-19D queue emitted out of order; the multiset oracle counts a duplicated accelerated row where `EXCEPT` did not (`ngram_search_differential.test` header); the decode-time interrupt surfaces where a stranded worker would hang the harness. No correctness assertion depends on machine speed (registry-scale timings are reports). Crash coverage runs on format 5 with 0 failures. Distribution matrix on 6325062: [34372906707](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34372906707) green on every target. Recorded runs: Correctness [34436332949](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34436332949) and nightly [34436333030](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34436333030) on 131c21a (row 22F names the lanes and what the first nightlies found). |
...
| Complete | Gate | Traceable release claims | `check --current-source`, `validate --binary` and `test --binary` pass on the evidence commit; every README example ran on the CLI with the documented output (Phase 23 review); links resolve; the stock-CLI install run is recorded (`docs/review/2026-09-09/install_check.log`). Correctness on the evidence commit d0a781b: [34376945939](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34376945939) (release suite, drivers, format check and evidence checks green; its sanitizer lane timed out on `ngram_parallel`, moved to the nightly lanes in 23ca25e); distribution matrix on d0a781b: [34376955783](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34376955783) green on every target; the later commits change only workflows, suppressions, the test README and this ledger, and Correctness is green on each (34391482857 failed on `ngram_batch_order` under ASAN before its move, then 34394125570, 34403566880, 34408156323, 34422370707, 34436332949); nightly [34436333030](https://github.com/danthegoodman1/duckdb-ngram/actions/runs/34436333030) as recorded in row 22F. `check --current-source` holds on every commit since d0a781b because none touches `src/**` or the tool. |
